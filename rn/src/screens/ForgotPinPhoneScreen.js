...
export default function ForgotPinPhoneScreen() {
  ...
  const handleSendOtp = async () => {
    ...
    setLoading(true);
    try {
      const result = await sendPinResetOtp(trimmed, selectedCountry.dialCode);
      if (result.success) {
        navigation.navigate('ForgotPinOtp', {
          phone: trimmed,
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Text style={[styles.title, { color: colors.textPrimary }]}>Forgot Transaction PIN?</Text>
        <Text style={[styles.subtitle, { color: colors.textSecondary }]}>
          Enter your registered phone number. We'll send you a one-time code to reset your PIN.
        </Text>
